...
function HiraganaQuiz() {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        const shuffledHiragana = shuffleArray(response.data.hiragana).slice(0, 10); 
        setHiragana(shuffledHiragana);
        setCurrentFlashcard(shuffledHiragana[0]);
      })
    ...
    if (currentFlashcard) {
      ...
      setTimeout(() => {
        ...
        if (nextIndex < hiragana.length) {
          setCurrentFlashcard(hiragana[nextIndex]);
          setUserInput('');
          setFeedback('');
  ...
  return (
    <div className="flashcard-quiz-container">
      <h1 className="quiz-title">Hiragana Quiz</h1>
